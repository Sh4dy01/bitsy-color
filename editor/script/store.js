...
var Store = {
	...
	get: function (name, fallback) {
		try {
            var value = this.getDriver().getItem(this.prefix + name);
	...
	set: function (name, value) {
		try {
			if (value === undefined) {
				this.getDriver().removeItem(this.prefix + name);
			} else {
                this.getDriver().setItem(this.prefix + name, JSON.stringify(value));
			}
		} catch (err) {
			if (!this.error && this.onFirstError) {
				this.onFirstError(err);
